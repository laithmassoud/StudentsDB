...
exports.findSubject = function(req, res) {
    //  Retrieve one user by Subject
    Student.find({"subjects": req.params.studentSubject}, function(err, student) {
        if(err) {
            res.status(500).send({message: "Could not retrieve student with subject " + req.params.studentSubject});
        } else {
            res.send(student);
        }
    });
...
exports.findByGender = function(req, res) {
    //  Retrieve one user by Subject
    Student.find({"gender": req.params.gender}, function(err, student) {
        if(err) {
            res.status(500).send({message: "Could not retrieve student with gender " + req.params.gender});
        } else {
            res.send(student);
        }
    });
...
exports.findByAge = function(req, res) {
    //  Retrieve one user by Subject
    Student.find({"age": req.params.age}, function(err, student) {
        if(err) {
            res.status(500).send({message: "Could not retrieve student with age " + req.params.age});
        } else {
            res.send(student);
        }
    });
...
exports.findByName = function(req, res) {
    ...
    Student.find({"name": req.params.name}, function(err, student) {
        if(err) {
            res.status(500).send({message: "Could not retrieve student with name " + req.params.name});
        } else {
            res.send(student);
        }
    });
